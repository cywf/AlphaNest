...
class VaultClient:
    ...
    def __init__(self):
        ...
        if not self.vault_token:
            logger.warning(
                "VAULT_TOKEN not set, using fallback to environment variables"
            )
    ...
    def _get_from_env(self, path: str, key: Optional[str] = None) -> Any:
        ...
        env_map = {
            ...
            "secret/data/jwt": {"secret": os.getenv("JWT_SECRET", "changeme")},
            "secret/data/openai": {"api_key": os.getenv("OPENAI_API_KEY")},
            "secret/data/tailscale": {"auth_key": os.getenv("TAILSCALE_AUTH_KEY")},
