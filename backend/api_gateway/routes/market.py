...
@router.post(
    "/booths", response_model=MarketBoothResponse, status_code=status.HTTP_201_CREATED
)
async def create_booth(
    ...
    existing_booth = (
        db.query(MarketBooth).filter(MarketBooth.user_id == user.id).first()
    )
    if existing_booth:
        raise HTTPException(
...
@router.post(
    "/nft", response_model=NFTItemResponse, status_code=status.HTTP_201_CREATED
)
