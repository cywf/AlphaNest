...
class User(Base):
    ...
    booth = relationship("MarketBooth", back_populates="user", uselist=False, cascade="all, delete-orphan")
...
class MarketBooth(Base):
    ...
    listings = relationship("NFTItem", back_populates="booth", cascade="all, delete-orphan")
...
class NFTItem(Base):
    ...
    booth_id = Column(String(36), ForeignKey("market_booths.id", ondelete="CASCADE"), nullable=False)
